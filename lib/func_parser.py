...
class FuncParser(object):
    ...
    def __init__(self):
        ...
        sin = pyp.CaselessLiteral("SIN")
        cos = pyp.CaselessLiteral("COS")
        tan = pyp.CaselessLiteral("TAN")
        ...
        # ident = pyp.Word(pyp.alphas, pyp.alphas + pyp.nums + "_$") | sin | cos | tan
        ident = sin | cos | tan
        plus = pyp.Literal("+")
        minus = pyp.Literal("-")
        mult = pyp.Literal("*")
        div = pyp.Literal("/")
        x = pyp.CaselessLiteral("X")
        ...
        atom = ((pyp.Optional(pyp.oneOf("- +")) +
                 (pi | e | fnumber | x | ident + lpar + expr + rpar).setParseAction(self.pushFirst))
    ...
    def is_valid(self, input_str: str) -> bool:
        try:
            self.bnf.parseString(input_str, True)
